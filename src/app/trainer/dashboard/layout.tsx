...
export default function TrainerLayout({ children }: { children: React.ReactNode }) {
    ...
    const menuItems = [
        {
            name: 'Overview', href: '/trainer/dashboard', icon: (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
            )
        },
        {
            name: 'Settings', href: '/trainer/dashboard/settings', icon: (
        ...
        <div className="min-h-screen bg-[#fcfcfc] flex">
                ...
                <div className="p-4 h-20 flex items-center border-b border-orange-500/30">
                    ...
                    {isSidebarOpen && (
                        <div className="ml-3 overflow-hidden whitespace-nowrap">
                            <h1 className="font-bold text-white text-sm tracking-tight leading-tight">HSGA</h1>
                            <p className="text-[10px] font-bold text-orange-200 uppercase tracking-widest leading-none mt-0.5">Telangana</p>
                        </div>
                    )}
                </div>
            ...
            <aside className={`fixed inset-y-0 left-0 z-[70] bg-orange-600 w-64 border-r border-orange-700 transition-transform duration-300 md:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-6 h-20 flex items-center border-b border-orange-500/30">
                    ...
                    <span className="ml-3 font-bold text-white tracking-tight">HSGA Telangana</span>
                </div>
                <nav className="p-4 space-y-1">
                    ...
                    <div className="pt-4 mt-4 border-t border-orange-500/30">
                        <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-3 py-3 text-red-600 bg-red-50 border border-red-100 rounded-lg font-black text-xs uppercase tracking-[0.1em] hover:bg-red-600 hover:text-white transition-all shadow-sm active:scale-95">
            ...
            <div className={`flex-grow flex flex-col min-w-0 transition-all duration-300 ${isSidebarOpen ? 'md:ml-64' : 'md:ml-20'}`}>
                <header className="h-16 bg-white border-b border-gray-100 px-4 md:px-8 flex items-center justify-between sticky top-0 z-40">
                    ...
                    <div className="flex items-center gap-2 md:gap-4">
                        ...
                        <button className="hidden sm:flex items-center gap-2 px-3 py-1.5 text-[10px] font-black text-gray-500 uppercase tracking-widest border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors">
                            Contact Support
                        </button>
